...
from spiderfoot import SpiderFootDb, SpiderFootEvent, SpiderFootPlugin, SpiderFootTarget, SpiderFootThreadPool, SpiderFootCorrelator, logger
...
class SpiderFootScanner():
    ...
    def __init__(self, scanName: str, scanId: str, targetValue: str, targetType: str, moduleList: list, globalOpts: dict, start: bool = True) -> None:
        ...
        if scanId:
            ...
            raise ValueError("scanId is empty")
    ...
    def __startScan(self) -> None:
        ...
        try:
            ...
            for modName in self.__moduleList:
                ...
                try:
                    # 동적으로 모듈을 import
                    # 'modules' 패키지 아래에 있는 modName 모듈을 불러옴
                    module = importlib.import_module(f"modules.{modName}")
                except ImportError:
                    # 모듈을 불러오지 못했을 때 에러 로그를 남기고 다음 모듈로 진행
                    self.__sf.error(f"Failed to load module: {modName}", exc_info=True)
                    continue

                try:
                    # 모듈 안에서 클래스 객체를 가져와 인스턴스화
                    mod_class = getattr(module, modName)
                    mod = mod_class()
                    # 모듈 이름을 객체에 저장 (추적 및 로깅용)
                    mod.__name__ = modName
                except Exception:
                    # 모듈 클래스 초기화 실패 시 에러 로그를 남기고 다음 모듈로 진행
        ...
        try:
            ...
            while True:
                ...
                if log_status:
                    scanstatus = self.__dbh.scanInstanceGet(self.__scanId)
                    if scanstatus and scanstatus[5] == "ABORT-REQUESTED":
                        self.__sf.info(f"{self.__scanId}: {counter}, ABORT-REQUESTED")
                        raise AssertionError("ABORT-REQUESTED")
            ...
            self.__sf.info("sharedThreadPool shutdown")
            try:
                self.__sharedThreadPool.shutdown(wait=False)
                self.__sf.info("sharedThreadPool shutdown success")
            except Exception as e:
                self.__sf.error("sharedThreadPool shutdown failed")
